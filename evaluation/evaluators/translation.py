...
class TranslationEvaluator:
    ...
    def get_nested_value(self, obj: Dict[str, Any], path: str) -> str:
        """
        get value of nested object, support dot-separated path
    ...
    def check_and_transform_with_template(self, test_json_file: str, template_file: Optional[str] = None) -> str:
        ...
        """
        logger.info(f"🔍 check and convert test json file: {test_json_file}")
        ...
        try:
            ...
            # if template file provided, use template to convert
            if template_file:
    ...
    def _resolve_test_file_path(self, test_input: str) -> str:
        """
        resolve test file path: support direct file path or find in dataset_info.json
        ...
        try:
            ...
            if test_input in dataset_info:
                ...
                if 'file_name' in dataset_config:
                    ...
                    if full_path.exists():
                        logger.info(f"✅ find file in dataset_info: {test_input} -> {full_path}")
            ...
            logger.error(f"❌ failed to resolve dataset_info.json: {e}")
        
        # if all above failed, try to find in data directory directly
        ...
        logger.warning(f"⚠️ failed to resolve file path, use original input: {test_input}")
        return test_input
    ...
    def _build_inference_args(self, 
        ...
        """
        build inference parameters list
        ...
        if few_shot:
            if model_config.get('finetuning_type') == 'lora':
                ...
                # full-finetuning model directly use trained model path
                inference_args = [
    ...
    def evaluation(self, 
        ...
        try:
            # 1. load prediction result and ground truth
...
def example_usage():
    ...
    # 2. do not use template, only default supplement (backward compatible)
    cmd, output_file = pipeline.inference(
        model_path="/path/to/trained/model",
        test_json_file="/path/to/test.json",
        model_config=model_config,
